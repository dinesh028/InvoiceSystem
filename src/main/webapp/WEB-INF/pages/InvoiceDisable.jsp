...
function disableInvoice(id)
{
	/* alert("FDSF FDSF "+id); */
		...
		$.ajax( {
			type : "POST",
			data: "id="+ id,
			url : "/InvoiceSystem/user/recurring/disable",
			cache : false,
			success : function(msg) {
...
<c:otherwise>
			 ...
			 <c:forEach items="${invoiceList}" var="invoice">                 
				...
				<td width="5%">${invoice.invoiceId}</td>
					<td width="5%">${invoice.invoiceParentId}</td>
			<td >${invoice.client.clientId}</td>
			<td >${invoice.client.name}</td>
			<td >${invoice.client.emailId}</td>
			<td ><input id=${invoice.invoiceId} type="button" value="Disable" onclick="disableInvoice(this.id)"/></td>
			<td > <input id=${invoice.invoiceId} type="button" value="View" onclick="javascript:GoTo(this.id)"  /></td>
