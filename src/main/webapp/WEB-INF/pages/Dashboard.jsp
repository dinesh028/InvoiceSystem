...
<link
	...
	<link href="/InvoiceSystem/js/dialogWindow.css" rel="stylesheet" type="text/css" />    
...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
 <script type="text/javascript" src="../js/jquery-1.8.0.min.js"></script>
  ...
  <script src="http://ajax.googleapis.com/ajax/libs/jquery/1.5/jquery.min.js"></script>
  <script src="http://ajax.googleapis.com/ajax/libs/jqueryui/1.8/jquery-ui.min.js"></script>
  <script src="/InvoiceSystem/js/FusionCharts.js"></script>
<script src="/InvoiceSystem//js/FusionCharts.jqueryplugin.js"></script>
 ...
 <script type="text/javascript" src="../js/FusionCharts.js">
	...
	<!-- no public hosting => embed dialog-extend v1.0.1 here -->
...
function graphDraw(){
	...
	$.ajax( {
		type : "GET",
		url : "/InvoiceSystem/user/dashboard/graph",
		data : "graphFormat=" + $("#graphFormat").val(),
		cache : false,
		success : function(msg) {
			 FusionCharts.setCurrentRenderer("javascript");
			var myChart = new FusionCharts("../js/MSColumn3D.swf","ChartId", "550", "280", "0");
			 
					myChart.setXMLData(msg);
					 var dialogOptions = {
				 			"title" : "Analytics",
				 			"width" : 600,
				 			"height" : 250,	
						 ...
						 myChart.setXMLData(msg);
						$("#chartContainer").dialog(dialogOptions).dialogExtend(dialogExtendOptions); 
 ...
 $(document).ready(function() {
		...
		$.ajax( {
			type : "GET",
			url : "/InvoiceSystem/user/dashboard/due",
			cache : false,
			success : function(msg) {
				displayDue(msg);
			},
 ...
 $(document).ready(function() {
		...
		$.ajax( {
			type : "GET",
			url : "/InvoiceSystem/user/dashboard/latest",
			cache : false,
			success : function(msg) {
				displayInvoice(msg);
			},
 ...
 function displayDue(data)
 {
     ...
     '<caption>Upcoming Due Invoices</caption>'+
     '<thead>'+
     '<tr hheight="44">'+
     '<th>Invoice Id</th>'+
     '<th>Client Name</th>'+
     ...
     '</tr>'+
     '</thead>';
     /* $.datepicker.formatDate(invoice.tenant.dateFormat, new Date(invoice.dueDate)) 
     alert( $.session("login") ); */
   $.each(data, function(i, invoice) {
     ...
     $("#divDue").dialog(dialogOptions1).dialogExtend(dialogExtendOptions);
     $("#divDue").html(table);
     $("#divDue br").remove(); 
 	//$("br").remove();
  ...
  function displayInvoice(data)
 {
     ...
     var table = '<table align="center" width="95%">'+
     '<caption>Latest Invoices</caption>'+
     '<thead >'+
     '<tr hheight="44">'+
     '<th>Invoice Id</th>'+
     '<th>Client Name</th>'+
     ...
     table += "<br/><br/><br/><br/><tr align = center  style=height:30px >"
     + "<td >" + invoice.invoiceId + " </td>"
     + "<td>" + invoice.cname + "</td>"
     + "<td>" + invoice.status + "</td>"
     + "<td>" + invoice.amount + "</td>"
 	...
 	//$("br").remove();
		...
		$("#divLatest").dialog(dialogOptions).dialogExtend(dialogExtendOptions);
		
		$("#divLatest").html(table);
		$("#divLatest br").remove();
 }	 
...
</div> <br><br><br><br><br><br><br><br><br><br>View Graph By:-
<select  id="graphFormat" onchange="graphDraw()">
 							<option value="1">Day</option>
  							<option value="2">Week</option>
  							<option value="3">Month</option>
  						</select><br>

<div align="left" id="chartContainer" style="height: 670px; width: 900px;" ></div>
